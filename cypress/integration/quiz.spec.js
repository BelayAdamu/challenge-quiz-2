...
describe('Quiz app test cases', () => {
  ...
  it('Should properly load the quiz app', () => {
    ...
    cy.get('h1')
      .should('have.text', 'Question 1 of 20')
    // Assert first question category
    cy.get('.header-wrapper > p')
      .should('have.text', unescape(questions[0].category))
    // Assert first question difficulty
    cy.get('.fa-star')
      .should('have.length', getStarCount(questions[0].difficulty))
    // Assert first question actual score
    cy.get('.txt-actual-score > p')
      .should('have.text', 'Score 0%')
    // Assert first question max score
    cy.get('.txt-max-score > p')
      .should('have.text', 'Max Score 100%')     
  })
  it('Should properly mark correct answer', () => {
    ...
    cy.wrap(questions[0].incorrect_answers).each(ans => {
      cy.contains(unescape(ans)).should('not.have.css', 'background-color', 'rgb(0, 0, 0)')
    })
    // All choices should be disabled
    cy.get('.choise-wrapper button')
      .should('be.disabled')
